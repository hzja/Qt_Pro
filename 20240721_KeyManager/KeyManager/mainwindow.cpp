    ...
    , ui(new Ui::MainWindow)
{
    ...
    //状态栏初始化创建
    labCurUser = new QLabel ("当前用户：", this);
    labCurUser->setMinimumWidth(250);
    ui->statusBar->addWidget (labCurUser) ; //加到状态栏
}
...
void MainWindow::on_setTableHead_clicked()
{
    ...
    QBrush *FontColor=new QBrush(Qt::red);
...
void MainWindow::on_LoginCheck_clicked()
{
    //创建新窗口
...
void MainWindow::loginSuc(const QString &username)
{
     //设置登录成功的槽函数，使登录成功后的用户名得以传递;
    labCurUser -> setText(QString::asprintf("当前用户：%s", username.toStdString().c_str()));
}
