...
void MainWindow::on_setTableHead_clicked()
{
    //设置表头
    QTableWidgetItem  *headerItem;
    QStringList headerText;
    QBrush *FontColor=new QBrush(Qt::black);
    headerText<<"姓 名"<<"性 别"<<"出生日期"<<"民 族"<<"分数"<<"是否党员";  //表头标题用QStringList来表示
    ui->tableInfo->setColumnCount(headerText.count());//列数设置为与 headerText的列数相等
...
//        font.setBold(true);//设置为粗体
        ...
        headerItem->setForeground(*FontColor);//字体颜色
        headerItem->setFont(font);//设置字体
        ui->tableInfo->setHorizontalHeaderItem(i,headerItem); //设置表头单元格的Item
    }
    ui -> tableInfo -> horizontalHeader() -> setSectionResizeMode(QHeaderView::Stretch); //表格充满布局
...
void MainWindow::on_btnIniTableData_clicked()
{
    ...
    QDate birth;
    birth.setDate(1998,11,22);//初始化一个日期
    ...
    for (int i=0;i<Rows;i++) //数据区第1行的编号为0，所以范围是0~rowCount()-1
    {
        ...
        birth=birth.addDays(300); //日期加20天
        isParty =!isParty;
    }
}
...
void MainWindow::on_FullScreenWin_clicked()
{
    ui -> tableInfo -> setGeometry(370,40,1500,400);//设置表格大小
    ui -> textEdit -> setGeometry(370,460,1500,100);//设置文本框大小
    this -> showFullScreen();//窗口全屏
}

void MainWindow::on_NormWin_clicked()
{
    ui -> tableInfo -> setGeometry(370,40,400,400);//设置表格大小
    ui -> textEdit -> setGeometry(370,460,400,100);//设置文本框大小
